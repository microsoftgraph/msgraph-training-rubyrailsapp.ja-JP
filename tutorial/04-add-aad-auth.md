...
`./app/controllers/application_controller.rb` ファイルを開きます。ここに、すべてのトークンを管理するメソッドを追加します。他のすべてのコントローラーは `ApplicationController` クラスを継承するため、これらのメソッドを使用してトークンにアクセスできるようになります。

次に示すメソッドを `ApplicationController` クラスに追加します。このメソッドは、OmniAuth ハッシュ値をパラメーターとして取得し、関連する情報の断片を抽出してからセッションに格納します。
...
次にユーザー名、メールアドレス、およびアクセストークンをセッションから取得するアクセッサーを追加します。
...
このメソッドは、ナビゲーションセクション要素内でユーザーを情報を表示するために（`application.html.erb` 内にある）レイアウトで使用される値を設定します。ここで追加することで、単一のコントローラーアクションごとにこのコードを追加する必要はありません。ただし、これは `AuthController` 内のアクションでも実行されますが、最適ではありません。`./app/controllers/auth_controller.rb` 内の `AuthController` クラスで before_action をスキップするために、次のコードを追加します。

```ruby
skip_before_action :set_user
```

その後、`AuthController` クラス内の `callback` 関数を、トークンをセッションに格納しメインページにリダイレクトするように修正します。 既に実装している `callback` 関数を、以下の内容に書き換えます。
